...
const SaveLoadModal: React.FC<SaveLoadModalProps> = ({ isOpen, onClose, mode, currentGameState, onLoadGame }) => {
  ...
  const checkAuthAndFetch = async () => {
      const currentUser = await Cloud.getCurrentUser();
      setUser(currentUser);
           ...
           <div className="flex items-center gap-1 sm:gap-2 shrink-0">
              ...
              {user ? (
                ...
                <div className="relative group/login-btn">
                  <button 
                    onClick={handleLogin}
                    className="text-xs uppercase px-3 py-1 font-bold transition-colors flex items-center gap-2 bg-scp-accent hover:bg-red-600 text-white"
                  >
                  ...
                  {isAIStudio && (
                    <div className="absolute top-full right-0 mt-2 w-48 px-2 py-1 bg-black border border-green-500/50 text-[10px] text-green-300 text-center opacity-0 group-hover/login-btn:opacity-100 transition-opacity pointer-events-none z-50 shadow-lg">
